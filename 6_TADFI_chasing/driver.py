...
def parse_slim(slim_string): 
    ...
    output += lines[4][9:] + "," + lines[5][7:] + ","+ lines[6][8:] + ","
    # Get the specified germline / embryo / fitness
    ...
    for i in range(0,len(lines)):
        if lines[i].startswith("Rates"):
            ...
            elif (float(rate_dr) - float(data[0])) > 0.05 and output.count(",") == 7:
                output += "1,"
...
def main():
    ...
    parser.add_argument('-src', '--source', default="mega_spatial0104_2D.slim", type=str,
    ...
    parser.add_argument('-introduce_ratio', '--INTRODUCE_RATIO', default=0.01, type=float,
                        help='The drop size of daisy drive . Default 0.7.')
  
    # drive parameters
    parser.add_argument('-embryo_cut_rate', '--EMBRYO_RESISTANCE_CUT_RATE_F_INPUT', default=0.0, type=float,
                       help='The embryo_resistance_rate of drive . Default 1.0.')
    
    parser.add_argument('-germline_cut_rate', '--WHOLE_GERMLINE_RESISTANCE_CUT_RATE', default=1.0, type=float,
                        help='The germline_resistance_rate of drive . Default 1.0')
    
    parser.add_argument('-Dwt_female_fitness', '--SOMATIC_FITNESS_MULTIPLIER_F', default=1.0, type=float,
                       help='The embryo_resistance_rate of drive . Default 1.0.')
    
    # ecology parameters
    parser.add_argument('-migration', '--AVERAGE_DISTANCE', default=0.08, type=float,
...
##add columns
    ...
    if args_dict.pop("print_header", None): # 需要和SLiM的输出match,代数=70
        print("radius,fraction,migration,growth,germline,embryo,fitness,invasiveness,suppressed_withoutChase,drive_lost_withoutChase,"\
